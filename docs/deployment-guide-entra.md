# Governance Platform Deployment Guide (Microsoft Entra ID)

End-to-end guide for deploying the EQTY Lab Governance Platform on Kubernetes with Microsoft Entra ID as the identity provider.
...
| **az**      | 2.50+           | Azure CLI (for Entra ID and Azure setup) |
...
```bash
./scripts/entra/bootstrap-entra.sh -f /path/to/bootstrap-values.yaml -n governance
```
...
```bash
# Watch job status
kubectl get jobs -l app.kubernetes.io/name=entra-bootstrap -n governance -w

# View logs
kubectl logs -l app.kubernetes.io/name=entra-bootstrap -n governance -f
```
...
kubectl logs -l app.kubernetes.io/name=entra-bootstrap -n governance
```
...
curl -s "https://login.microsoftonline.com/your-tenant-id/v2.0/.well-known/openid-configuration" | jq '.issuer'

# Expected output: "https://login.microsoftonline.com/your-tenant-id/v2.0"
...
> **Note:** The `graph-client-id` and `graph-client-secret` are typically the same as the backend app's `client-id` and `client-secret`, since the backend app registration has the `User.Read.All` Graph API permission needed for user lookups.
...
kubectl get secret platform-entra -n governance -o jsonpath='{.data}' | jq 'keys'
```
...
| `keyVault.azure.tenantId` | Azure AD tenant ID for Key Vault access | From your Azure subscription                         |
...
kubectl get jobs -n governance -l "app.kubernetes.io/component=entra-org-setup"

# View hook job logs if needed
kubectl logs -n governance -l "app.kubernetes.io/component=entra-org-setup" --tail=50
```
...
```bash
./scripts/entra/post-install-entra-setup.sh -n governance -e admin@yourorg.onmicrosoft.com
```
...
BACKEND_SECRET=$(kubectl get secret platform-entra -n governance -o jsonpath='{.data.client-secret}' | base64 -d)
BACKEND_CLIENT_ID=$(kubectl get secret platform-entra -n governance -o jsonpath='{.data.client-id}' | base64 -d)
TENANT_ID=$(kubectl get secret platform-entra -n governance -o jsonpath='{.data.tenant-id}' | base64 -d)
