# Governance Platform Deployment Guide (Keycloak + Azure)

End-to-end guide for deploying the EQTY Lab Governance Platform on Kubernetes with Keycloak as the identity provider, Azure Blob Storage for object storage, and Azure Key Vault for key management.
...
| **az**      | 2.50+           | Azure CLI (for Blob Storage, Key Vault)  |
...
export TENANT_ID=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx  # Azure AD tenant ID (for Key Vault)
```
...
2. Open `my-secrets.yaml` and:
   ...
   - Fill in all `REPLACE_WITH_*` values for Azure Blob Storage and Azure Key Vault credentials
...
>   platform-governance-worker platform-azure-blob \
>   platform-azure-key-vault platform-image-pull-secret platform-admin -n $NS 2>/dev/null
>
