# Governance Platform Deployment Guide (Auth0 + GCP)

End-to-end guide for deploying the EQTY Lab Governance Platform on Kubernetes with Auth0 as the identity provider, Google Cloud Storage for object storage, and Google Cloud KMS for key management.
...
```
1. Provision infrastructure (GCS, GCP KMS, DNS, TLS)
         │
...
- **Google Cloud Storage** — buckets for governance artifacts and integrity store, plus a service account with `objectAdmin` access
- **Google Cloud KMS** — a key ring in your chosen location, and a service account with `cloudkms.admin` (or granular key + sign + verify permissions) so the auth-service can create per-user DID signing keys at login time
...
Provision the following GCP resources before deploying. A running Kubernetes cluster with `kubectl` configured is assumed.
...
export ARTIFACTS_BUCKET=governance-artifacts            # GCS bucket for governance artifacts
...
> **Note:** If using GKE Workload Identity, skip this step — the service account credentials are provided automatically by the workload identity binding.
...
2. Open `my-secrets.yaml` and:
   ...
   - Fill in all `REPLACE_WITH_*` values for GCS and GCP KMS (paste your service account JSON keys)
...
1. Open `secrets-{env}.yaml` and fill in the remaining values marked with `# REQUIRED` comments:
   ...
   - GCP KMS service account JSON key (skip if using GKE Workload Identity)
...
auth-service:
  config:
    ...
    # Key Management — Google Cloud KMS for DID signing keys
    keyManagement:
      provider: "gcp_kms"
      gcp_kms:
...
| `keyManagement.gcp_kms.projectId`  | GCP project ID hosting the KMS key ring                   | Your GCP project (e.g., `your-gcp-project`)                    |
| `keyManagement.gcp_kms.locationId` | KMS location                                              | Region you used when creating the key ring (e.g., `us-east1`)  |
| `keyManagement.gcp_kms.keyRingId`  | KMS key ring name                                         | Name you used when creating the key ring (e.g., `eqtylab-did`) |
...
governance-service:
  config:
    ...
    storageProvider: "gcs"
    gcsBucketName: "governance-artifacts"
```
...
integrity-service:
  config:
    ...
    integrityAppBlobStoreGcsFolder: "" # Optional subfolder
```
...
- [ ] `governance-service.config.gcsBucketName` set
...
- [ ] `integrity-service.config.integrityAppBlobStoreGcsBucket` set
